...
class TableTest {
    ...
    @Test
    //Check if the tokens add;
    void Check_AddTokens(){
        //Before the add tokens;
        int size = table.curr_tokens;
        if(table.curr_tokens==0){
            table.placeToken(0, 0);
            //Check the size after the press;
            assertEquals(size+1, table.curr_tokens);
        }        
    }
    @Test
    //Check if the tokens remove;
    void Remove_Tokens(){
        //Before the remove tokens;
        int size = table.curr_tokens;
        if(table.curr_tokens==12){
            table.placeToken(0, 0);
            //Check the size after the press;
            assertEquals(size-1, table.curr_tokens);
        }
